...
UPDATES:
 - Hybrid mode:
   * Speed: ICSim-compatible (byte0 = speed in km/h). 
     Backwards-compatible: if byte0 not present but bytes 2-3 present, falls back to original 2-byte (value/100) format.
   * Blinkers: accepts ICSim-style byte0 bitmask (bit0=left, bit1=right) — compatible with original format.
   * Doors: KEEP original 4-byte door format (byte0..byte3).
"""
    ...
    def process_can_messages(self):
        ...
        while message_count < 10:  # Limit messages per frame
            ...
            if msg.arbitration_id == ID_SPEED:
                # HYBRID handling:
                # 1) Prefer ICSim-style: byte0 = speed (0-255 km/h)
                # 2) Fallback to original format: bytes 2-3 (big-endian), value/100 = km/h
                if len(msg.data) >= 1:
                    try:
                        ...
                        # if something weird, ignore and try fallback below
                        pass
                elif len(msg.data) >= 4:
                ...
                # This accepts ICSim-style values (0x00/0x01/0x02/0x03) and the original bitmask
                if len(msg.data) >= 1:
