...
usage: fucyfuzz dump [-h] [-f F] [-c] [-s SEC] [W ...]

CAN traffic dump module for Fucyfuzz
...
Example usage:
  fucyfuzz dump
  fucyfuzz dump -s 1.0
  fucyfuzz dump -f output.txt
  fucyfuzz dump -c -f output.txt 0x733 0x734

```
 
Example of __dumping to file all messages available on CAN bus:__
```
$ fucyfuzz dump -f output_all.txt      
...
```
_After ending the process with Ctrl+C we can see content saved to the file:_
```
$ cat output_all.txt

# Fucyfuzz dump file
# 2024-08-18 21:56:29.230679
# /usr/local/bin/fucyfuzz dump -f output_all.txt
...
```
$ fucyfuzz dump -f output_filtered.txt 0x0a7      
...
# /usr/local/bin/fucyfuzz dump -f output_filtered.txt 0x0a7
...
Example of __dumping only messages with filtered arbitration ID `0x0a7` with printing separating line after one second of silence on the bus:__
```
$ fucyfuzz dump -s 1 -f output_filtered1.txt 0x0a7      
...
```
_After ending the process with Ctrl+C we can see content saved to the file (one separating line can be spotted in the log file):_
```
$ cat output_filtered1.txt

# Fucyfuzz dump file
# 2024-08-21 21:04:28.420797
# /usr/local/bin/fucyfuzz dump -s 1 -f output_filtered1.txt 0x0a7
...
```
$ fucyfuzz dump -c -f output_filtered2.txt 0x0a7
...
# /usr/local/bin/fucyfuzz dump -c -f output_filtered2.txt 0x0a7
