...
This module can be used to enumerate and perform security testing on DoIP upon the ISO 13400-2. It is mainly build targeting the UDSonIP (ISO 14229-5) protocol implementation in Fucyfuzz, but it is supposed to be extended for further DoIP testing even outside the UDS implementation.

*Note: This module requires the packages `doipclient` and `udsoncan`, which only support Python 3.6+. These can be installed through `$ pip install doipclient udsoncan`.  If your system uses Python 2 as default python interpreter, you may have to use `$ pip3 install doipclient udsoncan` or `$ python3 -m pip install doipclient udsoncan` (and make sure to run fucyfuzz through `python3`) instead.*

The UDS protocol uses a server-client model, where the client (e.g. a diagnostics tool or fucyfuzz) sends requests on a specific arbitration ID, which a server (ECU) listens to. The server sends responses on another specific arbitration ID.
...
As always, module help can be shown by adding the `-h` flag (as shown below). You can also show help for a specific mode by specifying the mode followed by `-h`, e.g. `fucyfuzz doip discovery -h` or `fucyfuzz doip testerpresent -h`
...
usage: fucyfuzz doip [-h] {discovery,services,ecu_reset,testerpresent,security_seed,dump_dids,seed_randomness_fuzzer} ...

DoIP module for Fucyfuzz
...
Example usage:
  ...
  fucyfuzz doip discovery -blacklist 0x123 0x456
  fucyfuzz doip discovery -autoblacklist 10
  fucyfuzz doip services 0x733 0x633
  fucyfuzz doip ecu_reset 1 0x733 0x633
  fucyfuzz doip testerpresent 0x733
  fucyfuzz doip security_seed 0x3 0x1 0x733 0x633 -r 1 -d 0.5
  fucyfuzz doip dump_dids 0x733 0x633
  fucyfuzz doip dump_dids 0x733 0x633 --min_did 0x6300 --max_did 0x6fff -t 0.1
  fucyfuzz doip seed_randomness_fuzzer 2 2 0x733 0x633 -m 1 -t 10 -d 50 -id 4
```
...
usage: fucyfuzz doip discovery [-h] [-min MIN] [-max MAX] [-b B [B ...]] [-ab N] [-d D]
...
usage: fucyfuzz doip services [-h] [-t T] src dst
...
usage: fucyfuzz doip ecu_reset [-h] type src dst
...
usage: fucyfuzz doip testerpresent [-h] [-d D] [-dur S] src dst
...
usage: fucyfuzz doip dump_dids [-h] [-t T] [--min_did MIN_DID] [--max_did MAX_DID] src dst
...
```
$ fucyfuzz doip seed_randomness_fuzzer -h
...
usage: fucyfuzz doip seed_randomness_fuzzer [-h] [-t ITERATIONS] [-r RTYPE] [-id RTYPE] [-m RMETHOD] [-d D] stype level src dst
