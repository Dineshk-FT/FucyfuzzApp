...
The best way to understand how to use fucyfuzz is to look at the help screen:
...
fucyfuzz is based on a main entry point in `fucyfuzz.py` which runs the show. This enables an easy drop-in architecture for new modules, which are located in the `fucyfuzz/modules` folder.

The `fucyfuzz/utils` folder contains various higher level CAN protocol implementations and shared functions, meant to be used by modules.

The `fucyfuzz/tests` folder contains automated test suites and `/documentation` stores documentation files (modules are also documented here).

## List of Modules
A clean installation of fucyfuzz includes the following modules:
...
### test - Run test suite
Runs automated fucyfuzz test suites

## List of libraries/utilities
The `fucyfuzz/utils` folder contains the following:
...
- A template for new modules is available in `fucyfuzz/modules/module_template.py`
- Create a python file with a function `module_main(args)` (or copy the template) in the `fucyfuzz/modules` directory.
- In `setup.py`, add an entry under `fucyfuzz.modules`, referencing your new module like: `my_module = fucyfuzz.modules.my_module`
- Run `python setup.py install`
- Verify that the module is available, it should be listed in the output of `fucyfuzz -h`

If your new module is located in `fucyfuzz/modules/foo.py` you will run it with the command `fucyfuzz foo`.
