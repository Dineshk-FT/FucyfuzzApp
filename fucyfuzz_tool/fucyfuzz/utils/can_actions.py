...
import fucyfuzz.utils.constants as constants
import can
import time
...
# Global CAN interface setting, which can be set through the -i flag to fucyfuzz.py
...
class CanActions:
    ...
    def __init__(self, arb_id=None, notifier_enabled=True, interface=None):
        """
        ...
        :param interface: str CAN interface to use (e.g., 'vcan0', 'can0')
        """
        if interface is None:
            interface = DEFAULT_INTERFACE
        self.bus = can.Bus(interface)
        self.arb_id = arb_id
