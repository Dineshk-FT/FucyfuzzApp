...
from fucyfuzz.utils.common import hex_str_to_nibble_list, int_from_byte_list, list_to_hex_str, parse_int_dec_or_hex
from fucyfuzz.utils.constants import ARBITRATION_ID_MAX, ARBITRATION_ID_MIN, BYTE_MAX, BYTE_MIN
...
def random_fuzz(static_arb_id=None, static_data=None, filename=None, min_id=ARBITRATION_ID_MIN,
                ...
                start_index=0, show_status=True, seed=None, interface=None):
    """
    ...
    try:
        ...
        with CanActions(interface=interface) as can_wrap:
            ...
            while True:
                ...
                # Send message with Success/Fail check
                status_str = "SUCCESS"
                try:
                    ...
                    status_str = "FAIL"
                    # Optionally print specific error if debugging:
                    # print(f"\nError sending: {e}")

                if show_status:
                    # Included [STATUS] in the carriage return print
                    print("\r[{0}] Messages sent: {1}, index: {2}".format(status_str, messages_sent, current_index), end="")
                    stdout.flush()
...
def bruteforce_fuzz(arb_id, initial_data, data_bitmap, filename=None, start_index=0, show_progress=True,
    ...
    try:
        if file_logging_enabled:
            output_file = open(filename, "a")
        with CanActions(arb_id=arb_id, interface=interface) as can_wrap:
            if show_progress:
            ...
            for current_fuzzed_nibbles in fuzz_data:
                ...
                message_count += 1
                if show_progress:
                    print("\r[{0}] Current: {1} Index: {2}".format(status_str, list_to_hex_str(output_data, " "), message_count), end="")
                    stdout.flush()
                # Log to file
                if file_logging_enabled:
    ...
    try:
        ...
        with CanActions() as can_wrap:
            ...
            while True:
                ...
                if show_status:
                    # We will print status after attempting to send, or optimistically here but update success later?
                    # Let's calculate status first.
                    pass
                ...
                if show_status:
                    print("\r[{0}] Sending {1:04x} # {2} ({3} sent, index {4})".format(
                        status_str, arb_id, list_to_hex_str(data, " "), messages_sent, current_index), end="")
                    stdout.flush()
...
def replay_fuzz(directives, show_requests, show_responses):
    ...
    with CanActions() as can_wrap:
        ...
        for arb_id, data in directives:
            ...
            if show_requests:
                print("[{0}] Sending ({1}) {2}".format(status_str, count, directive))
            sleep(DELAY_BETWEEN_MESSAGES)
        ...
        try:
            ...
            while True:
                ...
                for index in range(len(composites)):
                    ...
                    print("[{0}] Sending ({1}/{2}) {3}".format(status_str, index+1, len(composites), directive))
                    sleep(DELAY_BETWEEN_MESSAGES)
...
def __handle_random(args):
    ...
    random_fuzz(static_arb_id=args.id, static_data=data, filename=args.file,
                min_data_length=args.min, max_data_length=args.max, start_index=args.index, seed=args.seed, interface=args.interface)
...
def __handle_bruteforce(args):
    ...
    bruteforce_fuzz(arb_id=args.arb_id, initial_data=data, data_bitmap=data_bitmap, 
                   filename=args.file, start_index=args.index, show_progress=True, 
                   show_responses=args.responses, interface=args.interface)
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(prog="fucyfuzz fuzzer",
                                     formatter_class=argparse.RawDescriptionHelpFormatter,
                                     description="Fuzzing module for Fucyfuzz",
...
fucyfuzz fuzzer identify log.txt""")
    ...
    cmd_random.add_argument("--interface", "-if", default="vcan0", help="CAN interface to use")
    ...
    # Add this to your argument parsers
    cmd_brute.add_argument("--interface", "-if", default="vcan0", help="CAN interface to use")
...
def module_main(arg_list):
    ...
    :param arg_list: Module argument list passed by fucyfuzz.py
    """
    try:
        # Parse arguments
