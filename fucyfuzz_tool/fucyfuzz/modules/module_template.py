...
This file contains a template for a simple Fucyfuzz module.
The module's entry point is the 'module_main' function.

Steps to add this module to Fucyfuzz and run it:

1. Copy this template into the `fucyfuzz/modules` directory:

    $ cp module_template.py my_module.py

2. In `setup.py`, add an entry under `fucyfuzz.modules`,
   referencing your new module like:
   `my_module = fucyfuzz.modules.my_module`

3. Run: `setup.py install`

4. Verify that the module is available,
   it should be listed in the output of `fucyfuzz -h`

5. Run the following command to run module and show usage instructions:

    $ fucyfuzz my_module -h
"""
import argparse
import time
...
def parse_args(args):
    ...
    """
    parser = argparse.ArgumentParser(prog="fucyfuzz module_template",
                                     formatter_class=argparse.RawDescriptionHelpFormatter,
                                     description="Descriptive message for the template module",
                                     epilog="""Example usage:
  fucyfuzz module_template
  fucyfuzz module_template -id 123
  fucyfuzz module_template -id 0x1FF""")
...
def module_main(arg_list):
    """
    Module main wrapper. This is the entry point of the module when called by fucyfuzz

    :param arg_list: Module argument list passed by fucyfuzz
    """
    try:
        # Parse arguments
